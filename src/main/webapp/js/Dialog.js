...
/**
 * 提示对话框。 类似于HTML元素上的title提示。创建提示对话框可以通过实例化对话框对象，也可以通过提示对话框的辅助 工具进行注册。
 * 
 * example: new HintDialog($('text'));将产生一个默认提示对话框，与HTML ID为text的HTML元素绑定，
 * 无内容、默认宽度、默认触发事件(即创建实例的时候，显示对话框)。 new HintDialog($('text'), 'This a hint
 * dialog', {width: '300px', showEvent: 'click'}); 将产生一个提示对话框，与HTML
 * ID为text的HTML元素绑定，宽度为300px，并且是在点击该HTML元素后显示对话框。
 * 
 * HintDialog.register($('text'));注册一个默认提示对话框。 HintDialog.register($('text'),
 * 'This a hint dialog', {width: '300px', showEvent: 'click'});
 * 注册一个有内容、设置选项的提示对话框。
 * 
 * 提示对话框引用到的样式，默认定义在dialog.css中： hint-dialog-top-line：对话框顶部线条样式
 * hint-dialog-arrow：对话框箭头样式 hint-dialog-content：对话框内容部分样式
...
HintDialog.prototype = {
	...
	initialize : function(obj, msg, options) {
		...
		if (this._options.showEvent != null) {
			Event.observe(this._obj, this._options.showEvent, this._showHint
							.bindAsEventListener(this), false);
		} else {
			this._showHint();
		}

		Event.observe(this._obj, this._options.hideEvent, this._hideHint
						.bindAsEventListener(this), false);
	...
	_generateContent : function() {
		...
		// src=\"images/text_content-left.gif\" /></div>"
		// + "<div style=\"float:left; height:24px;
		// background:url(images/text_content-bg.gif); line-height:24px;
		// color:#265972;\">"
		// + this._msg
		// + "</div>"
		// + "<div style=\"float:left;\"><img
		// src=\"images/text_content-right.gif\" /></div>"
		// + "</div>"
		// + "<div style=\"clear:both;\"></div>"
		// + "<div style=\"margin-left:40px; width:80px;
		// margin-top:-42px;\"><img src=\"images/top_angle.gif\" /></div>";
		// }
