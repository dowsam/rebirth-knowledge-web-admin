dhtmlXGridObject.prototype.enableBlockSelection = function(mode) {
    if (typeof this._bs_mode == "undefined") {
        ...
        this.attachEvent("onRightClick", function (code, ctrl, shift) {
            this.pasteBlockFromClipboard();
            return false;
        });
    };
...
dhtmlXGridObject.prototype.pasteBlockFromClipboard = function() {
    ...
    for (var i = startRow; i < endRow; i++) {
        ...
        for (var j = startCol; j < endCol; j++) {
            ...
            if (!ed.grid.callEvent("onEditCell", [0, row.idd, j])) {
                l++;
                continue; //2010.03.25 by jeff liu
            }
...
dhtmlXGridObject.prototype.dragOrBlockSelection = function (mode) {
    if (!this.isActive) {
        this.setActive(true);
    }
    if (mode > 0) {
        ...
        this.enableSmartXMLParsing(false);
        this.disableBlockSelection();
        this.dragger._bs_mode = false;
    } else {
        this.enableDragAndDrop(false);
        this.enableSmartXMLParsing(false);
        this.enableBlockSelection();
        this.dragger._bs_mode = this._bs_mode;
    }
};