    ...
    for (var i = 0; i < this.rowsBuffer.length; i++)
        if (!this.rowsBuffer[i]) {
            if (this.callEvent("onDynXLS", [i, buffer])) {
                var self = this;
                this.load(this.xmlFileUrl + getUrlSymbol(this.xmlFileUrl) + "pageNo=" + i + "&pageSize=" + buffer, function() {
            ...
            if (res == -1) {
                if (this.xmlFileUrl) {
                    ...
                    if (this.callEvent("onDynXLS", [j, this._current_load[1]]))
                        this.load(this.xmlFileUrl + getUrlSymbol(this.xmlFileUrl) + "pageNo=" + j + "&pageSize=" + this._current_load[1], this._data_type)
                };
                return
            } else {
                if (this._tgle) {
