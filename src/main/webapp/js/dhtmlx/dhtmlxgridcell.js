...
eXcell_roDiv.prototype = new eXcell;

//扩展img
function eXcell_imglink(cell) {
	this.base = eXcell_img;
	this.base(cell);
	this.getValue = function () {
		var ids = "";
		if (this.cell.childNodes) {
			for (var i = 0; i < this.cell.childNodes.length; i++) {
				if (this.cell.childNodes[i]) {
					var url = this.cell.childNodes[i].href;
					if (url) {
						var s = url.split("=");
						ids = ids + s[1] + ";";
					}
...
eXcell_imglink.prototype = new eXcell_img;
eXcell_imglink.prototype.setValue = function (val) {
	var title = "";
	if (val == undefined || val == "" || val == "^") {
		...
		for (var i = 0; i < iddd.length; i++) {
			...
			imgs = imgs + "<a href='" + this.grid.srchref + iddd[i] + "' target='_blank'><img fileId='" + iddd[i] + "' src='" + this.grid.srchref + iddd[i] + "' alt='" + a + "' class='px80' onload='imagespx80(this)' border='0'></a>";
			if (i < 5) {
				imgs = imgs + "<br />";
			}
		}
		this.setCValue(imgs, val);
        //this.dq("<img src='clientproject!projectDetailsPicture.action?fileId="+val+"' border='0'>", val);
		if (iddn) {
			this.cell.tit = iddn.join(";");
		} else {
			this.cell.tit = "";
		}
	}
...
eXcell_number.prototype = new eXcell_ed;
eXcell_number.prototype.setValue = function(val) {
    if (isNaN(Number(val))) {
        val = this.val || 0
    };
    var color = "green";
