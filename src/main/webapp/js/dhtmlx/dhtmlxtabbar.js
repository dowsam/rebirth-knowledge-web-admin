...
dhtmlXTabBar.prototype = {
		...
		if (name.indexOf("dhx_sky") == 0) {
			...
			var f = function() {
				...
				var w = this._tabAll[this._s._vMode ? "offsetHeight"
						...
						+ (_quirks ? 2 : 0);
				if (this._lastActive) {
					if(this._lastActive.parentNode[this._s._vMode ? "scrollHeight"
												: "scrollWidth"]){
						w = Math
								.max(
										w,
	...
	_img : function(tag, y, pos, a, b) {
		tag.style.backgroundImage = "url(" + this.imgUrl + this.skin + "/"
				+ tag.parentNode.skin + "_" + this._s.mode + ".gif)";
	...
	_onClickHandler : function(e) {
		...
		if (target && !target._disabled)
			...
			if (tag) {
				...
				tag.nextSibling.style.right = tag.parentNode.scrollLeft * (-1)
						+ "px";
				return false
			}
		}
		...
		if (!this._content[id]) {
			...
			d.show = function() {
				...
				if (self._awdj || self._ahdj) {
					...
					if (self._ahdj)
						self.entBox.style.height = dim[1] + self._rows.length
								* (self._s.line_height - 2) + 2 + "px";
					self._setSizes();
