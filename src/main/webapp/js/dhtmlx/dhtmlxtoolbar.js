function dhtmlXToolbarObject(baseId, skin) {
	var main_self = this;
	this.cont = (typeof (baseId) != "object") ? document.getElementById(baseId)
			: baseId;
	...
	this.setRTL = function(mode) {
		...
		this.cont.className = "dhx_toolbar_base_" + this.skin
				+ (this.rtl ? " rtl" : "");
		...
		for ( var a in this.objPull) {
			...
			if (item["type"] == "buttonSelect")
				item.polygon.className = "dhx_toolbar_poly_" + this.skin
						+ (this.rtl ? " rtl" : "");
			if (item["type"] == "slider")
				item.label.className = className = "dhx_toolbar_slider_label_"
						+ this.skin + (this.rtl ? " rtl" : "")
		}
	};
	...
	this.setSkin = function(skin) {
		...
		for ( var a in this.objPull) {
			...
			if (item["type"] == "slider") {
				...
				item.pen._definePos();
				item.label.className = "dhx_toolbar_slider_label_" + this.skin
						+ (this.rtl ? " rtl" : "")
			}
			;
			if (item["type"] == "buttonSelect") {
	...
	this._addItemToStorage = function(itemData, pos) {
		var id = (itemData.getAttribute("id") != null ? itemData
				.getAttribute("id") : this._genStr(24));
		var type = (itemData.getAttribute("type") != null ? itemData
				.getAttribute("type") : "");
		if (type != "") {
			if (this["_" + type + "Object"] != null) {
				this.objPull[this.idPrefix + id] = new this["_" + type
						+ "Object"](this, id, itemData);
	...
	this.rootTypes = new Array("button", "buttonSelect", "buttonTwoState",
			"separator", "label", "slider", "text", "buttonInput");
	...
	this._getObj = function(obj, tag) {
		...
		for ( var q = 0; q < obj.childNodes.length; q++) {
			if (obj.childNodes[q].tagName != null) {
				if (String(obj.childNodes[q].tagName).toLowerCase() == String(
						tag).toLowerCase())
	...
	this._enableItem = function(item) {
		if (item.state)
			return;
		item.state = true;
		if (this.objPull[item.id]["type"] == "buttonTwoState"
				&& this.objPull[item.id]["obj"]["pressed"] == true) {
		...
		;
		if (item.arw)
			item.arw.className = String(item.obj.className).replace("btn",
					"arw");
		var imgObj = this._getObj(item.obj, "img");
		if (item.imgEn != "") {
			if (imgObj == null)
	...
	dhtmlxEvent(
			...
			function(e) {
				if (!main_self.base)
					return;
				main_self
						.forEachItem(function(itemId) {
							if (main_self.objPull[main_self.idPrefix + itemId]["type"] == "buttonSelect") {
								var item = main_self.objPull[main_self.idPrefix
										+ itemId];
								if (item.polygon.style.display != "none") {
									item.obj.renderAs = "dhx_toolbar_btn def";
									item.obj.className = item.obj.renderAs;
									item.arw.className = String(
											item.obj.renderAs).replace("btn",
											"arw");
...
dhtmlXToolbarObject.prototype.addButton = function(id, pos, text, imgEnabled,
		imgDisabled) {
	var itemText = (text != null ? (text.length == 0 ? null : text) : null);
	var str = '<item id="' + id + '" type="button"'
			+ (imgEnabled != null ? ' img="' + imgEnabled + '"' : '')
			+ (imgDisabled != null ? ' imgdis="' + imgDisabled + '"' : '')
			+ (itemText != null ? ' text="' + itemText + '"' : "") + '/>';
...
dhtmlXToolbarObject.prototype.addButtonSelect = function(id, pos, text, opts,
		imgEnabled, imgDisabled) {
	var itemText = (text != null ? (text.length == 0 ? null : text) : null);
	var str = '<item id="' + id + '" type="buttonSelect"'
			+ (imgEnabled != null ? ' img="' + imgEnabled + '"' : '')
			+ (imgDisabled != null ? ' imgdis="' + imgDisabled + '"' : '')
			+ (itemText != null ? ' text="' + itemText + '"' : "") + '>';
	for ( var q = 0; q < opts.length; q++) {
		if (opts[q].id && opts[q].type) {
			if (opts[q].type == "obj") {
				str += '<item type="button" id="'
						+ opts[q].id
						+ '" text="'
						+ opts[q].text
						+ '"'
						+ (opts[q].img != null ? ' img="' + opts[q].img + '"'
								: '') + '/>'
...
dhtmlXToolbarObject.prototype.addButtonTwoState = function(id, pos, text,
		imgEnabled, imgDisabled) {
	var itemText = (text != null ? (text.length == 0 ? null : text) : null);
	var str = '<item id="' + id + '" type="buttonTwoState"'
			+ (imgEnabled != null ? ' img="' + imgEnabled + '"' : '')
			+ (imgDisabled != null ? ' imgdis="' + imgDisabled + '"' : '')
			+ (itemText != null ? ' text="' + itemText + '"' : "") + '/>';
	this._addItem(str, pos)
...
dhtmlXToolbarObject.prototype.addSlider = function(id, pos, len, valueMin,
		valueMax, valueNow, textMin, textMax, tip) {
	var itemTextMin = (textMin != null ? (textMin.length == 0 ? null : textMin)
			: null);
	var itemTextMax = (textMax != null ? (textMax.length == 0 ? null : textMax)
			: null);
	var str = '<item id="' + id + '" type="slider" length="' + len
			+ '" valueMin="' + valueMin + '" valueMax="' + valueMax
			+ '" valueNow="' + valueNow + '"'
			+ (itemTextMin != null ? ' textMin="' + itemTextMin + '"' : '')
			+ (itemTextMax != null ? ' textMax="' + itemTextMax + '"' : '')
			+ ' toolTip="' + tip + '"/>';
	this._addItem(str, pos)
};
dhtmlXToolbarObject.prototype.addInput = function(id, pos, value, width) {
	var str = '<item id="' + id + '" type="buttonInput" value="' + value
			+ '" width="' + width + '"/>';
	this._addItem(str, pos)
};
...
(function() {
	var list = "showItem,hideItem,isVisible,enableItem,disableItem,isEnabled,setItemText,getItemText,setItemToolTip,getItemToolTip,setItemImage,setItemImageDis,clearItemImage,clearItemImageDis,setItemState,getItemState,setItemToolTipTemplate,getItemToolTipTemplate,setValue,getValue,setMinValue,getMinValue,setMaxValue,getMaxValue,setWidth,getWidth"
			.split(",");
	var ret = [ "", "", false, "", "", false, "", "", "", "", "", "", "", "",
	...
	var functor = function(name, res) {
		return function(itemId, a, b) {
			...
			if (this.objPull[itemId][name] != null)
				return this.objPull[itemId][name].call(this.objPull[itemId], a,
						b);
			else
				return res
		}
...
dhtmlXToolbarObject.prototype._setPosition = function(id, pos) {
	...
	if (newPos[0] == null) {
		...
		if (item.arw)
			this.base.insertBefore(item.arw,
					this.base.childNodes[newPos[1] + 1])
	}
};
	...
	var functor = function(name) {
		return function(parentId, a, b, c, d, e) {
			...
			if (this.objPull[parentId]["type"] != "buttonSelect")
				return;
			return this.objPull[parentId][name].call(this.objPull[parentId], a,
					b, c, d, e)
		}
	};
	for ( var i = 0; i < list.length; i++) {
...
dhtmlXToolbarObject.prototype._separatorObject = function(that, id, data) {
	...
	this.obj.title = (data.getAttribute("title") != null ? data
			.getAttribute("title") : "");
...
dhtmlXToolbarObject.prototype._buttonObject = function(that, id, data) {
	...
	this.imgEn = (data.getAttribute("img") != null ? data.getAttribute("img")
			: "");
	this.imgDis = (data.getAttribute("imgdis") != null ? data
			.getAttribute("imgdis") : "");
	this.img = (this.state ? (this.imgEn != "" ? this.imgEn : "")
			: (this.imgDis != "" ? this.imgDis : ""));
	this.obj = document.createElement("DIV");
	...
	this.obj.innerHTML = (this.img != "" ? "<img src='" + that.imagePath
			+ this.img + "'>" : "")
			+ (data.getAttribute("text") != null ? "<div>"
					+ data.getAttribute("text") + "</div>" : "");
	var obj = this;
	this.obj.onselectstart = function(e) {
		e = e || event;
		e.returnValue = false
...
dhtmlXToolbarObject.prototype._buttonSelectObject = function(that, id, data) {
	this.id = that.idPrefix + id;
	this.state = (data.getAttribute("enabled") != null ? (data
			.getAttribute("enabled") == "true" ? true : false) : true);
	this.imgEn = (data.getAttribute("img") != null ? data.getAttribute("img")
			: "");
	this.imgDis = (data.getAttribute("imgdis") != null ? data
			.getAttribute("imgdis") : "");
	...
	this.obj.title = (data.getAttribute("title") != null ? data
			.getAttribute("title") : "");
	this.obj.pressed = false;
	this.renderSelect = (data.getAttribute("renderSelect") != null ? (data
			.getAttribute("renderSelect") == "false"
			|| data.getAttribute("renderSelect") == "disabled" ? false : true)
			: true);
	this.obj.innerHTML = (this.img != "" ? "<img src='" + that.imagePath
			+ this.img + "'>" : "")
			+ (data.getAttribute("text") != null ? "<div>"
	...
	this.arw.onmousedown = function(e) {
		...
		if (that.anyUsed == self.obj.idd) {
			self.obj.className = self.obj.renderAs;
			self.arw.className = String(self.obj.renderAs)
					.replace("btn", "arw");
			that.anyUsed = "none";
			...
			if (that.anyUsed != "none") {
				if (that.objPull[that.idPrefix + that.anyUsed]["type"] == "buttonSelect") {
					...
					if (item.polygon.style.display != "none") {
						item.obj.renderAs = "dhx_toolbar_btn def";
						item.obj.className = item.obj.renderAs;
						item.arw.className = String(self.obj.renderAs).replace(
								"btn", "arw");
						item.polygon.style.display = "none";
						if (item.polygon._ie6cover)
							item.polygon._ie6cover.style.display = "none"
			...
			if (that.rtl) {
				...
				self.polygon.style.left = getAbsoluteLeft(self.obj)
						+ self.obj.offsetWidth - self.polygon.offsetWidth
						+ that.selectPolygonOffsetLeft + "px";
				self.polygon.style.left = getAbsoluteLeft(self.obj)
						+ self.obj.offsetWidth - self.polygon.offsetWidth
						+ that.selectPolygonOffsetLeft + "px"
			} else {
			...
			;
			self.polygon.style.top = getAbsoluteTop(self.obj)
					+ self.obj.offsetHeight + that.selectPolygonOffsetTop
					+ "px";
			if (that.rtl) {
			...
			if (self.polygon._ie6cover) {
				...
				self.polygon._ie6cover.style.width = self.polygon.offsetWidth
						+ "px";
				self.polygon._ie6cover.style.height = self.polygon.offsetHeight
						+ "px";
	...
	this._separatorButtonSelectObject = function(id, data, pos) {
		...
		if (isNaN(pos)) {
			...
			if (pos > self.polygon.childNodes.length) {
				...
				self.polygon.insertBefore(this.obj,
						self.polygon.childNodes[pos - 1])
			}
		}
		;
	...
	this._buttonButtonSelectObject = function(id, data, pos) {
		...
		this.obj.className = "btn_item"
				+ (this.obj.en ? (this.obj._selected && self.renderSelect ? " sel"
						: "")
						: " dis");
		this.obj.onselectstart = function(e) {
			e = e || event;
		...
		var itemText = (data.getAttribute("text") != null ? data
		...
		;
		this.obj.innerHTML = (data.getAttribute("img") != null ? "<img src='"
				+ that.imagePath + data.getAttribute("img") + "' border='0'>"
				: "")
				+ "<span>" + itemText + "</span>";
		this.obj.onmouseover = function(e) {
			if (!this.en)
		...
		this.obj.onmouseout = function(e) {
			if (!this.en)
				return;
			this.className = "btn_item"
					+ (this._selected && self.renderSelect ? " sel" : "")
		};
		this.obj.idd = String(id);
		this.obj.onclick = function(e) {
			e = e || event;
	...
	this.polygon.className = "dhx_toolbar_poly_" + that.skin
			+ (that.rtl ? " rtl" : "");
	...
	for ( var q = 0; q < data.childNodes.length; q++) {
		if (data.childNodes[q].tagName == "item") {
			var id = (data.childNodes[q].getAttribute("id") != null ? data.childNodes[q]
					.getAttribute("id")
					: that._genStr(24));
			var type = (data.childNodes[q].getAttribute("type") != null ? "_"
					+ data.childNodes[q].getAttribute("type")
					+ "ButtonSelectObject" : that._genStr(24));
			if (this[type] != null) {
				that.objPull[that.idPrefix + id] = new this[type](id,
						data.childNodes[q]);
				that.objPull[that.idPrefix + id]["type"] = "buttonSelectNode";
				that.objPull[that.idPrefix + id]["typeext"] = (String(type)
						.search("separator") != -1 ? "separator" : "button")
			}
		}
	}
	;
	...
	this.setWidth = function(width) {
		...
		this.polygon.style.width = this.obj.offsetWidth + this.arw.offsetWidth
				- 2 + "px"
	};
	this.enableItem = function() {
		that._enableItem(this)
	...
	this.addListOption = function(id, pos, type, text, img) {
		...
		that.objPull[that.idPrefix + id] = new this["_" + type
				+ "ButtonSelectObject"](id, data.childNodes[0], pos);
		that.objPull[that.idPrefix + id]["type"] = "buttonSelectNode";
	...
	this.setListOptionPosition = function(id, pos) {
		...
		if (pos > this.polygon.childNodes.length) {
			...
			this.polygon.insertBefore(that.objPull[id].obj,
					this.polygon.childNodes[pos - 1])
		}
	};
	...
	this.setListOptionImage = function(id, img) {
		...
		if ((that.objPull[id].obj.childNodes[0].tagName).toLowerCase() == "img") {
			...
			imgObj.border = "0";
			that.objPull[id].obj.insertBefore(imgObj,
					that.objPull[id].obj.childNodes[0])
		}
		;
...
dhtmlXToolbarObject.prototype._buttonInputObject = function(that, id, data) {
	...
	this.obj.className = "dhx_toolbar_btn def";
	this.obj.idd = String(id);
	this.obj.w = (data.getAttribute("width") != null ? data
			.getAttribute("width") : 100);
	this.obj.title = (data.getAttribute("title") != null ? data
			.getAttribute("title") : "");
	this.obj.innerHTML = "<input class='inp' type='text' style='width:"
			+ this.obj.w
			+ "px;'"
			+ (data.getAttribute("value") != null ? "' value='"
...
dhtmlXToolbarObject.prototype._buttonTwoStateObject = function(that, id, data) {
	...
	this.obj.className = "dhx_toolbar_btn "
			+ (this.obj.pressed ? (this.state ? "pres" : "dis")
					: (this.state ? "def" : "dis"));
	this.obj.renderAs = this.obj.className;
	this.obj.idd = String(id);
	this.obj.title = (data.getAttribute("title") != null ? data
	...
	this.obj.onmousedown = function(e) {
		if (that.checkEvent("onBeforeStateChange"))
			if (!that.callEvent("onBeforeStateChange", [
					this.idd.replace(that.idPrefix, ""), this.pressed ]))
				return;
		if (obj.state == false)
			return;
		if (that.anyUsed != "none")
		...
		that.callEvent("onStateChange", [ this.idd.replace(that.idPrefix, ""),
				this.pressed ]);
		return false
	};
	this.setItemState = function(state, callEvent) {
		if (this.obj.pressed != state) {
			...
			;
			if (callEvent == true)
				that.callEvent("onStateChange", [
						this.obj.idd.replace(that.idPrefix, ""),
						this.obj.pressed ])
		}
	};
...
dhtmlXToolbarObject.prototype._sliderObject = function(that, id, data) {
	...
	this.obj.idd = String(id);
	this.obj.len = (data.getAttribute("length") != null ? Number(data
			.getAttribute("length")) : 50);
	this.obj.innerHTML = "<div>"
			+ (data.getAttribute("textMin") != null ? data
					.getAttribute("textMin") : "")
			+ "</div>"
			+ "<div class='sl_bg_l'></div>"
			+ "<div class='sl_bg_m' style='width:"
			+ this.obj.len
			+ "px;'></div>"
			+ "<div class='sl_bg_r'></div>"
			+ "<div>"
			+ (data.getAttribute("textMax") != null ? data
					.getAttribute("textMax") : "") + "</div>";
	that.base.appendChild(this.obj);
	var self = this;
	...
	this.label.className = "dhx_toolbar_slider_label_" + that.skin
			+ (that.rtl ? " rtl" : "");
	this.label.style.display = "none";
	this.label.tip = (data.getAttribute("toolTip") != null ? data
			.getAttribute("toolTip") : "%v");
	document.body.appendChild(this.label);
	var label = this.label;
	this.pen.valueMin = (data.getAttribute("valueMin") != null ? Number(data
			.getAttribute("valueMin")) : 0);
	this.pen.valueMax = (data.getAttribute("valueMax") != null ? Number(data
			.getAttribute("valueMax")) : 100);
	if (this.pen.valueMin > this.pen.valueMax)
		this.pen.valueMin = this.pen.valueMax;
	this.pen.valueNow = (data.getAttribute("valueNow") != null ? Number(data
			.getAttribute("valueNow")) : this.pen.valueMax);
	...
	this.pen._definePos = function() {
		this.nowX = Math.round((this.valueNow - this.valueMin)
				* (this.maxX - this.minX) / (this.valueMax - this.valueMin)
				+ this.minX);
	...
	this.pen.onmousedown = function(e) {
		...
		if (label.tip != "") {
			...
			label.style.left = Math.round(getAbsoluteLeft(this)
					+ this.offsetWidth / 2 - label.offsetWidth / 2)
					+ "px";
			label.style.top = getAbsoluteTop(this) - label.offsetHeight - 3
					+ "px";
	...
	this.pen._doOnMouseMoveStart = function(e) {
		...
		if (e.clientX < getAbsoluteLeft(pen) + Math.round(pen.offsetWidth / 2)
				&& pen.nowX == pen.minX)
			return;
		if (e.clientX > getAbsoluteLeft(pen) + Math.round(pen.offsetWidth / 2)
				&& pen.nowX == pen.maxX)
		...
		pen.newValueNow = Math.round((pen.valueMax - pen.valueMin)
				* (pen.newNowX - pen.minX) / (pen.maxX - pen.minX)
				+ pen.valueMin);
		if (label.tip != "") {
			label.innerHTML = label.tip.replace(/%v/gi, pen.newValueNow);
			label.style.left = Math.round(getAbsoluteLeft(pen)
					+ pen.offsetWidth / 2 - label.offsetWidth / 2)
					+ "px";
			label.style.top = getAbsoluteTop(pen) - label.offsetHeight - 3
					+ "px"
		}
	};
	this.pen._doOnMouseMoveEnd = function() {
		...
		that.callEvent("onValueChange", [
				self.obj.idd.replace(that.idPrefix, ""), pen.valueNow ])
	};
	if (_isIE) {
	...
	this.setValue = function(value, callEvent) {
		...
		if (callEvent == true)
			that.callEvent("onValueChange", [
					this.idd.replace(that.idPrefix, ""), this.pen.valueNow ])
	};
	this.getValue = function() {
...
dhtmlXToolbarObject.prototype.unload = function() {
	...
	for ( var a in obj) {
		...
		if (el["type"] == "buttonSelect") {
			...
			if (el.polygon._ie6cover) {
				if (el.polygon._ie6cover.parentNode)
					el.polygon._ie6cover.parentNode
							.removeChild(el.polygon._ie6cover);
		...
		if (el["type"] == "slider") {
			...
			var pen = el.pen;
			if (_isIE) {
				document.body.detachEvent("onmousemove",
						pen._doOnMouseMoveStart);
				document.body.detachEvent("onmouseup", pen._doOnMouseMoveEnd)
			} else {
				window.removeEventListener("mousemove",
						pen._doOnMouseMoveStart, false);
				window.removeEventListener("mouseup", pen._doOnMouseMoveEnd,
						false)
			}
			;
			pen = null;
	...
	this.base = null;
	this.tr = null;
	var list = new Array("showItem", "hideItem", "isVisible", "enableItem",
			"disableItem", "isEnabled", "setItemText", "getItemText",
			"setItemToolTip", "getItemToolTip", "setItemImage",
			"setItemImageDis", "clearItemImage", "clearItemImageDis",
			"setItemState", "getItemState", "setItemToolTipTemplate",
			"getItemToolTipTemplate", "setValue", "getValue", "setMinValue",
			"getMinValue", "setMaxValue", "getMaxValue", "setWidth",
			"getWidth", "_addItem", "_doOnLoad", "_setLayout", "_string2xml",
			"_xmlLoader", "getType", "getTypeExt", "inArray", "addButton",
			"addText", "addButtonSelect", "addButtonTwoState", "addSeparator",
			"addSlider", "addInput", "forEachItem", "_addItemToStorage",
			"_buttonInputObject", "_buttonObject", "_buttonSelectObject",
			"_buttonTwoStateObject", "_genStr", "_getPosition",
			"_separatorObject", "_setPosition", "_sliderObject", "_textObject",
			"_xmlParser", "addListOption", "attachEvent", "callEvent",
			"checkEvent", "clearListOptionImage", "detachEvent",
			"disableListOption", "enableListOption", "eventCatcher",
			"forEachListOption", "getAllListOptions", "getListOptionImage",
			"getListOptionPosition", "getListOptionSelected",
			"getListOptionText", "getListOptionToolTip", "getPosition",
			"hideListOption", "isListOptionEnabled", "_getIdByPosition",
			"isListOptionVisible", "loadXML", "loadXMLString", "removeItem",
			"removeListOption", "setIconPath", "setIconsPath",
			"setListOptionImage", "setListOptionPosition",
			"setListOptionSelected", "setListOptionText",
			"setListOptionToolTip", "setPosition", "showListOption",
			"dhx_Event");
	...
	dhtmlx.extend_api("dhtmlXToolbarObject", {
		...
		items : function(arr) {
			for ( var i = 0; i < arr.length; i++) {
				var item = arr[i];
				if (item.type == "button")
					this.addButton(item.id, null, item.text, item.img,
							item.img_disabled);
				...
				if (item.type == "buttonSelect")
					this.addButtonSelect(item.id, null, item.text,
							item.options, item.img, item.img_disabled);
				if (item.type == "buttonTwoState")
					this.addButtonTwoState(item.id, null, item.text, item.img,
							item.img_disabled);
				if (item.type == "buttonInput")
					this.addInput(item.id, null, item.text);
				if (item.type == "slider")
					this.addSlider(item.id, null, item.length, item.value_min,
							item.value_max, item.value_now, item.text_min,
							item.text_max, item.tip_template);
				if (item.width)
