...
dhtmlXGridObject.prototype.showWaitingBox = function(message) {
	if (this.messageBox == undefined) {
		this.messageBox = document.getElementById("UpdateProgress1");
...
dhtmlXGridObject.prototype.refresh = function(callback) {
	...
	this.clearAndLoad(xmlFileUrl, function(){callback(grid, cp, Math.ceil(grid.rowsBuffer.length/grid.rowsBufferOutSize))},this.dataType);
}
...
dhtmlXGridObject.prototype.initSetting = function(setting) {
	...
	var widthUnit=setting.widthUnit;
	if("px"==widthUnit){
		calls[1]="setInitWidths";
	}
	this.dataType=setting.dataType || "json";
	// 初始化grid的配置
