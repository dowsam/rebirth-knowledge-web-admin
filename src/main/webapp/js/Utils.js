...
Utils.isIE = (/msie/i.test(navigator.userAgent) && !/opera/i.test(navigator.userAgent));
 ...
 * 
 * @param HTMLEelement
 *            el 需要屏蔽龌龊元素的HTMLElement对象
 */
Utils.hideShowCovered = function (el) {
	function getVisib(obj) {
		var value = obj.style.visibility;
		if (!value) {
			if (document.defaultView && typeof (document.defaultView.getComputedStyle) == "function") { // Gecko,
				// W3C
				if (!Utils.isKHtml) {
...
/***
 * 限制input只能输入数字
 * 例如:<input type="text" onkeypress="javascript:return isNumber(event);"/>
 * @param {} e
 * @return {Boolean}
 */
