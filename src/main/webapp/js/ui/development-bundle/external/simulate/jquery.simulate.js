	...
	$.fn.extend({
				simulate : function(type, options) {
					return this.each(function() {
								var opt = $.extend({}, $.simulate.defaults,
										options || {});
	...
	$.extend($.simulate.prototype, {
		...
		mouseEvent : function(type, options) {
			...
			if ($.isFunction(document.createEvent)) {
				...
				evt.initMouseEvent(type, e.bubbles, e.cancelable, e.view,
		...
		drag : function(el) {
			var self = this, center = this.findCenter(this.target), options = this.options, x = Math
					.floor(center.x), y = Math.floor(center.y), dx = options.dx
					|| 0, dy = options.dy || 0, target = this.target;
			var coord = {
				clientX : x,
				clientY : y
			};
