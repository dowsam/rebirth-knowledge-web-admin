 ...
 * 
 * Copyright (c) 2008 John Resig, Jörn Zaefferer Dual licensed under the MIT
...
(function($) {
	...
	var equiv = function() {
		...
		var callbacks = function() {
			...
			return {
				...
				"regexp" : function(b, a) {
					return hoozit(b) === "regexp" && a.source === b.source
							&& // the regex itself
							a.global === b.global
	...
	var GETParams = $.map(location.search.slice(1).split('&'),
			decodeURIComponent), ngindex = $.inArray("noglobals", GETParams), noglobals = ngindex !== -1;
	...
	$(window).load(function() {
		...
		var head = $('<div class="testrunner-toolbar"><label for="filter-pass">Hide passed tests</label></div>')
				.insertAfter("#userAgent");
		$('<input type="checkbox" id="filter-pass" />').attr("disabled", true)
				.prependTo(head).click(function() {
							$('li.pass')[this.checked ? 'hide' : 'show']();
						});
		$('<input type="checkbox" id="filter-missing">').attr("disabled", true)
				.appendTo(head).click(function() {
					$("li.fail:contains('missing test - untested code is broken code')")
							.parent('ol').parent('li.fail')[this.checked
							? 'hide'
							: 'show']();
				});
		$("#filter-missing")
				.after('<label for="filter-missing">Hide missing tests (untested code is broken code)</label>');
		runTest();
	});
	...
	function runTest() {
		...
		synchronize(function() {
					$('<p id="testresult" class="result"/>').html([
							'Tests completed in ', +new Date - started,
							' milliseconds.<br/>', '<span class="bad">',
							config.stats.bad,
							'</span> tests of <span class="all">',
							config.stats.all, '</span> failed.'].join(''))
							.appendTo("body");
	...
	function checkPollution(name) {
		...
		if (pollution.length > old.length) {
			ok(false, "Introduced global variable(s): "
							+ diff(old, pollution).join(", "));
			config.expected++;
		}
	}
	...
	function test(name, callback) {
		...
		synchronize(function() {
					...
					try {
						...
						QUnit.ok(false, "Setup failed on " + name + ": "
										+ e.message);
					}
				})
		synchronize(function() {
		...
		synchronize(function() {
			try {
				...
				if (typeof console != "undefined" && console.error
						&& console.warn) {
					console.error("reset() failed, following Test " + name
			...
			if (config.expected && config.expected != config.assertions.length) {
				QUnit.ok(false, "Expected " + config.expected
								+ " assertions, but "
			...
			for (var i = 0; i < config.assertions.length; i++) {
				...
				$("<li/>").addClass(assertion.result ? "pass" : "fail")
						.text(assertion.message || "(no message)").appendTo(ol);
				assertion.result ? good++ : bad++;
			}
			config.stats.bad += bad;

			var b = $("<strong/>").html(name
					+ " <b style='color:black;'>(<b class='fail'>" + bad
					+ "</b>, <b class='pass'>" + good + "</b>, "
					+ config.assertions.length + ")</b>").click(function() {
						$(this).next().toggle();
			...
			$("<li/>").addClass(bad ? "fail" : "pass").append(b).append(ol)
					.appendTo("#tests");
	...
	function isSet(a, b, msg) {
		...
		if (a && b && a.length != undefined && a.length == b.length) {
			...
			ret = false;
		QUnit.ok(ret, !ret ? (msg + " expected: " + serialArray(b)
						+ " result: " + serialArray(a)) : msg);
	}
	 ...
	 * 
	 * @example
	 * equals( $.format("Received {0} bytes.", 2), "Received 2 bytes." );
	 * 
	 * @param Object
	 *            actual
	 * @param Object
	...
	function push(result, actual, expected, message) {
		...
		QUnit.ok(result, result ? message + ": " + expected : message
						+ ", expected: " + jsDump.parse(expected) + " result: "
						+ jsDump.parse(actual));
	}
...
(function() {
	...
	function join(pre, arr, post) {
		var s = jsDump.separator(), base = jsDump.indent();
	...
	var jsDump = window.jsDump = {
		...
		typeOf : function(obj) {
			var type = typeof obj, f = 'function';// we'll use it 3 times,
													// save it
			return type != 'object' && type != f
					? type
					: !obj
							? 'null'
							: obj.exec ? 'regexp' : // some browsers (FF)
													// consider regexps
													// functions
									obj.getHours
											? 'date'
		...
		parsers : {
			...
			'function' : function(fn) {
				var ret = 'function', name = 'name' in fn ? fn.name : (reName
			...
			key : quote, // object calls it internally, the key part of an
							// item in a map
			functionCode : '[code]', // function calls it internally, it's
