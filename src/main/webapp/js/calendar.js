...
function $showMsg(text, timeout) {
	...
	if (typeof timeout != "undefined")
		window.setTimeout($hideMsg, timeout);
}
function $hideMsg() {
...
function $toggleDisplay(e) {
	var id = (typeof e == "object") ? e.id : e;
	Utility.toggleVisibility(id);
};
function $focus(e) {
	if (typeof e == "object")
		e.focus();
...
$toggle = function(btnId, divId) {
	...
	if ($isVisible(divId)) {
		$nodisplay(divId);
		btn.className = TOGGLE_ICON_EXPAND;
	} else {
		$display(divId);
		btn.className = TOGGLE_ICON_COLLAPSE;
	}
}
...
var Utility = {
	...
	shiftOpacity : function(id, millisec) {
		if ($(id).style.opacity == 0) {
			Utility.opacity(id, 0, 100, millisec);
		} else {
	...
	blockUI : function() {
		$display('blockUI');
		$('blockUI').style.height = Math.max(PU.getContentHeight(), "1000")
				+ "px";
	},
	// Utility.unblockUI()
	unblockUI : function() {
	...
	exec : function(text) {
		text = text.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g,
				"\\n").replace(/\r/g, "\\n");
		setTimeout('try { eval("' + text
						+ '"); } catch(ex) { alert(ex.message); }', 0);
	}
};
...
document
		.write("#calendarTable{width:179px;margin:0;padding:0;border:1px solid #395366;background:#fff;}");
document.write("th,td{margin:0;padding:0px}");
document
		.write("#calendarTable th ,#calendarTable td{font:12px/18px 宋体,Arial,sans-serif;text-align:center; color:#395366; background:#F9F9FA;}");
document.write("#calendarTable thead th.week{background:#E2E7EF;}");
document
		.write("#calendarTable thead tr.function th{color:#61801D!important; font-weight:bold;background:#B7C4D4};");
document
		.write("#calendarTable thead tr.function th a{color:#091150!important; font-weight:bold;}");
document
		.write("#calendarTable thead tr.function th a:hover{color:#FFF!important; font-weight:bold;}");
document.write("#calendarTable thead tr.top{letter-spacing:1px;}");
document
		.write("#calendarTable thead a{color:#051350;width:19px;height:18px;text-decoration:none;display:block; }");
...
function writeDate() {
	...
	if (tempYear != toDay.getFullYear() || tempMonth != toDay.getMonth())
		today = -1;
	var todayStyle = "";
	var weekEndStyle = "";
	clear();
	var intDaysInMonth = getDays(curCal.getMonth(), curCal.getFullYear());
	var weeks = (intDaysInMonth + startDay) % 7 == 0
			? (intDaysInMonth + startDay) / 7
			: parseInt((intDaysInMonth + startDay) / 7) + 1;
	...
	for (var intWeek = 1; intWeek <= weeks; intWeek++) {
		...
		for (var intDay = 0; intDay < 7; intDay++) {
			...
			if ((daily > 0) && (daily <= intDaysInMonth)) {
				if (language == "en") {
					td.innerHTML = "<a href=\"javascript:void(0);\" class=\""
							+ weekEndStyle + todayStyle
							+ "\" onclick=\"getDate('" + daily
							+ "')\" title=\"" + eval(tempMonth + 1) + "-"
							+ daily + "\">" + daily + "</a>";
				} else if (language == "zhbig5") {
					td.innerHTML = "<a href=\"javascript:void(0);\" class=\""
							+ weekEndStyle + todayStyle
							+ "\" onclick=\"getDate('" + daily
							+ "')\" title=\"" + eval(tempMonth + 1) + "月"
							+ daily + "日\">" + daily + "</a>";
				} else {
					td.innerHTML = "<a href=\"javascript:void(0);\" class=\""
							+ weekEndStyle + todayStyle
...
function getDate(day) {
	...
	if (day == "0") {
		...
		month = eval(toDay.getMonth() + 1) < 10 ? "0"
				+ eval(toDay.getMonth() + 1) : eval(toDay.getMonth() + 1);
		date = toDay.getDate() < 10 ? "0" + toDay.getDate() : toDay.getDate();
	} else {
		year = tempYear;
		month = eval(tempMonth + 1) < 10
				? "0" + eval(tempMonth + 1)
				: eval(tempMonth + 1);
...
function vent(event) {
	if (document.all) {
		if (isShow) {
			if (window.event.srcElement != objouter
		...
		if (isShow) {
			...
			if (event.target != objouter && event.target.tagName != 'IMG'
