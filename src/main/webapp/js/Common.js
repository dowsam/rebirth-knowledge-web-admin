 ...
 * * 判断是否是Gmail电子邮箱
 */
String.prototype.isGEmail = function() {

	try {
		var pattern = /^\w+[\w+.]*@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/;
		var pattern2 = /^(([a-z A-Z \- _ . 0-9]+)@[a-z A-Z \- _ 0-9]+\.([a-z A-Z]+(\.)?)?[a-z A-Z]+)$/;
		return pattern.test(this.Trim()) || pattern2.test(this.Trim());
	} catch (e) {
		return false;
	}

}
Common.isGEmail = function(id) {
	var obj = $(id);
	try {
		if (obj.value.isGEmail()) {
			return true;
		} else {
			obj.focus();
			return false;
...
Common.isDomain = function(obj) {
	...
	if (list[list.length - 1].RealLength() > 4
			|| list[list.length - 1].RealLength() < 2)
		return false;
...
function SetTabPageHW(item) {
	try {
		...
		top.document.getElementById(item).style.height = (objHeight > 450
				? objHeight
				: 450)
				+ "px";
	} catch (e) {
