...
(function(jQuery) {
	/*
	 * jQuery Plugin - Messager Author: corrie Mail: corrie@sina.com Homepage:
	 * www.corrie.net.cn Copyright (c) 2008 corrie.net.cn @license
	 * http://www.gnu.org/licenses/gpl.html [GNU General Public License]
	 * 
	 * $Date: 2008-12-26 $Vesion: 1.4 @ how to use and example: Please Open
	 * demo.html
	 */
	this.version = '@1.3';
	this.layer = {
	...
	this.inits = function(title, text) {
		if (jQuery("#message").is("div")) {
			return;
		}
		jQuery(document.body)
				.prepend('<div id="message" style="border:#b9c9ef 1px solid;z-index:100;width:'
						+ this.layer.width
						+ 'px;height:'
						+ this.layer.height
	...
	this.show = function(title, text, time) {
		...
		switch (this.anims.type) {
			case 'slide' :
				jQuery("#message").slideDown(this.anims.speed);
				break;
			case 'fade' :
				jQuery("#message").fadeIn(this.anims.speed);
				break;
			case 'show' :
				jQuery("#message").show(this.anims.speed);
				break;
			default :
				jQuery("#message").slideDown(this.anims.speed);
				break;
	...
	this.close = function() {
		switch (this.anims.type) {
			case 'slide' :
				jQuery("#message").slideUp(this.anims.speed);
				break;
			case 'fade' :
				jQuery("#message").fadeOut(this.anims.speed);
				break;
			case 'show' :
				jQuery("#message").hide(this.anims.speed);
				break;
			default :
				jQuery("#message").slideUp(this.anims.speed);
				break;
